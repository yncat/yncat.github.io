...
次は ssh の設定です。手元で ssh-keygen とかやってキー pair を作ります。作ったら、ダッシュボードから行ける、さっき作ったインスタンスの書斎画面を開きます。「編集」というのを押します。「ssh」タブを押します。
...
80 番に来た通信を、大富豪サーバーに転送してくれる設定をします。デフォルトでは、大富豪サーバーは 2567 番のポートを使うので、 2567 に送ってあげます。
